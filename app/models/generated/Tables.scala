...
trait Tables {
  ...
  implicit def GetResultUserRow(implicit e0: GR[Int], e1: GR[Option[String]], e2: GR[Option[java.sql.Date]], e3: GR[String], e4: GR[java.sql.Timestamp], e5: GR[Boolean]): GR[UserRow] = GR{
    prs => import prs._
    UserRow.tupled((<<[Int], <<?[String], <<?[String], <<?[String], <<?[java.sql.Date], <<[String], <<[String], <<[String], <<[java.sql.Timestamp], <<[Boolean], <<[java.sql.Timestamp]))
  }
  ...
  class User(_tableTag: Tag) extends profile.api.Table[UserRow](_tableTag, Some("myappdb"), "user") with IdentifyableTable[Int] {
              def * = (id, firstName, middleName, lastName, dateOfBirth, username, email, avatarUrl, lastLogin, active, modified) <> (UserRow.tupled, UserRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), firstName, middleName, lastName, dateOfBirth, Rep.Some(username), Rep.Some(email), Rep.Some(avatarUrl), Rep.Some(lastLogin), Rep.Some(active), Rep.Some(modified))).shaped.<>({r=>import r._; _1.map(_=> UserRow.tupled((_1.get, _2, _3, _4, _5, _6.get, _7.get, _8.get, _9.get, _10.get, _11.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
