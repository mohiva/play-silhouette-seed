...
class ScratchCodeDaoSpec extends DaoSpecLike {
  ...
  "The scratch code dao" should {
    "should create code correctly" in new Context {
      val (user, totpInfoOpt): (UserRow, Option[GoogleTotpInfo]) = for {
        user <- userDao.createAndFetch(testUser)
        _ <- loginInfoDao.create(user.id, testLoginInfo)
        _ <- totpInfoDelegableDao.add(testLoginInfo, testGoogleTotpInfo.copy(scratchCodes = Seq(testScratchCode.copy(userId = user.id).toExt)))
